...
public class UserController {
  ...
  public String importUser(@RequestParam("file") MultipartFile file, Model model) throws IOException, InvalidFormatException {
    if (file.getSize() > 100) {
      List<User> list = excelUtils.getList(file.getInputStream());
      userDetailsService.saveAll(list);
      LOG.info("添加用户：" + list.toString());
